  ...
  return (
    <div className="flex flex-col items-center justify-center py-8 px-2 sm:px-4 progress-container">
      <div className="w-full max-w-2xl bg-primary rounded-xl shadow-lg p-8 flex flex-col items-center border-4 border-black">
        <h1 className="text-3xl font-bold mb-4 text-center">{translate("progress")}</h1>
        <div className="mb-6 w-full">
          <h2 className="text-xl font-semibold mb-2">
            {currentLanguage === "es" ? "Porcentaje total de progreso" : "Total progress percentage"}
          </h2>
        ...
        <div className="mb-6 w-full">
          <h2 className="text-xl font-semibold mb-2">
            {currentLanguage === "es" ? "Lista de cursos completados" : "Completed courses list"}
          </h2>
          {completedCourseObjs.length === 0 ? (
            <p>{currentLanguage === "es" ? "No has completado ningún curso todavía." : "You haven't completed any courses yet."}</p>
          ) : (
            <ul className="pl-0">
        ...
        <div className="w-full">
          <h2 className="text-xl font-semibold mb-2">
            {currentLanguage === "es" ? "Progreso por categoría o nivel" : "Progress by category or level"}
          </h2>
          {progressByCategory.length === 0 ? (
            <p>{currentLanguage === "es" ? "No hay datos de progreso por categoría." : "No progress data by category."}</p>
          ) : (
